...
#include "Poisson.h"
   ...
   */
void Poisson::jacobian (const NumericVector<Number> & soln,
 ...
 */
void Poisson::residual (const NumericVector<Number> & soln,
      ...
      for (unsigned int qp=0; qp<qrule.n_points(); qp++)
        {
          ...
          Real fxy = rhs_value(q_point[qp], 0);

          for (unsigned int dof_i=0; dof_i<n_dofs; dof_i++){
            
            Re(dof_i) += JxW[qp] * dphi[dof_i][qp] * du_value - JxW[qp] * phi[dof_i][qp] * fxy;       
